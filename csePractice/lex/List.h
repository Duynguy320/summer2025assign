...
typedef struct List{
	Node* cursor; //currently selected node
	Node* front;
	Node* back;
	int length;
	int index;
...
//creates and returns the pointer to a node with
//the given data
...
// in: N/A
// out: Empty list struct
void freeList(List* pL); 
// Frees all heap memory associated with *pL, and sets
// *pL to NULL

 // Access functions -----------------------------------------------------------
int getLength(List* L); // Returns the number of elements in L.
int getIndex(List* L); // Returns index of cursor element if defined, -1 otherwise
int getFront(List* L); // Returns front element of L. Pre: length()>0
int getBack(List* L); // Returns back element of L. Pre: length()>0
int getCursor(List* L); // Returns cursor element of L. Pre: length()>0, index()>=0
...
void printList(FILE* out, List* L); // Prints to the file pointed to by out, a
 // string representation of L consistin
